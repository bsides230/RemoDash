...
# --- Mocking TTS for Sandbox/No-GPU environments ---
try:
    import torch
    import torchaudio
    ...
    logger.warning(f"Coqui TTS not found ({e}). Running in MOCK MODE.")
    MOCK_MODE = True

    # Mock Class
    class TTS:
        def __init__(self, model_name, progress_bar=False, gpu=False):
            logger.info(f"Mock TTS Initialized: {model_name}")
            self.device = "cpu"
        ...
        def tts_to_file(self, text, speaker_wav, language, file_path, gpt_cond_latent=None, speaker_embedding=None):
            logger.info(f"Mock TTS Generating: {text} -> {file_path}")
            # Create a dummy silent WAV file
            # minimal valid wav header (44 bytes)
            with open(file_path, 'wb') as f:
                # RIFF header
                f.write(b'RIFF')
                f.write((36).to_bytes(4, 'little')) # Size
                f.write(b'WAVE')
                # fmt chunk
                f.write(b'fmt ')
                f.write((16).to_bytes(4, 'little')) # chunk size
                f.write((1).to_bytes(2, 'little')) # format (PCM)
        ...
        def get_conditioning_latents(self, audio_path):
            logger.info(f"Mock TTS Calculating Latents for {audio_path}")
...
async def add_voice(name: str = Form(...), audio: UploadFile = File(...)):
    if not tts_instance and not MOCK_MODE:
